...
import { accessControlStatus, verifyTeamWorkspaceSession, verifyWorkspaceAccess } from './access-control.js';
import { createDemoReport } from './demo-report.js';
import {
  exportPayload,
  exportableApprovedTasks,
  graphTrace,
  issueExportMode,
  logEntry,
  preflightStatus,
...
async function routeApiRequest({ method, pathname, query = {}, body = {}, headers = {} }) {
  ...
  const access = verifyWorkspaceAccess({ pathname, headers, workspaceId: workspaceContext.id });
  if (!access.ok) return access.response;
  const storage = getStorage(workspaceContext.id);
  const requestProvider = providerStatus({ access: access.status.effectiveMode });

  if (method === 'GET' && pathname === '/api/team/workspaces') {
    const status = accessControlStatus();
    return {
      status: 200,
      body: {
        configured: status.configured || status.teamWorkspaces.configured,
        access: status.configured ? 'global-guarded' : status.teamWorkspaces.configured ? 'team-guarded' : 'demo-open',
        teams: status.teamWorkspaces.teams,
      },
    };
  }

  if (method === 'POST' && pathname === '/api/team/session') {
    const session = verifyTeamWorkspaceSession({
  ...
  if (method === 'GET' && pathname === '/api/workspace') {
    return { status: 200, body: decorateWorkspace(await storage.getWorkspace(), requestProvider, access.status) };
  }
  ...
  if (method === 'POST' && pathname === '/api/runs/select') {
    ...
    return { status: 200, body: decorateWorkspace(await storage.selectRun(runId), requestProvider, access.status) };
  }

  if (method === 'DELETE' && pathname === '/api/workspace') {
    return { status: 200, body: decorateWorkspace(await storage.resetWorkspace(), requestProvider, access.status) };
  }

  if (method === 'GET' && pathname === '/api/llm/free-models') {
    return { status: 200, body: { ...(await getConfiguredFreeModels()), provider: requestProvider } };
  }
  ...
  if (method === 'GET' && pathname === '/api/export-package') {
    ...
    return {
      status: 200,
      body: createIssueExportPackage(target, await storage.getWorkspace(), requestProvider),
    };
  }
  ...
  if (method === 'POST' && pathname === '/api/agent/run') {
    ...
    return { status: 200, body: decorateWorkspace(await runProductAgent({ idea, storage }), requestProvider, access.status) };
  }
  ...
  if (method === 'PATCH' && pathname === '/api/tasks/batch') {
    ...
    return { status: 200, body: decorateWorkspace(await storage.patchTasks(taskIds, patch, graph, log), requestProvider, access.status) };
  }
  ...
  if (method === 'PATCH' && taskMatch) {
    ...
    return { status: 200, body: decorateWorkspace(await storage.patchTask(taskId, patch, graph, log), requestProvider, access.status) };
  }
  ...
  if (method === 'POST' && pathname === '/api/export') {
    ...
    const tasksToExport = exportableApprovedTasks(target, workspace.tasks, workspace.exports);
    const payload = exportPayload(target, workspace.prd, tasksToExport);
    if (!payload.length) {
      return { status: 409, body: { error: `All approved tasks have already been exported to ${target}.` } };
    }

    const provider = requestProvider;
    let delivery = null;
    let exportStatus = 'payload-only';
    const mode = issueExportMode(target, provider);
    ...
    const exportRecord = {
      ...
      taskIds: tasksToExport.map((task) => task.id),
      payload,
      mode,
      delivery,
    };
    ...
    return {
      status: 200,
      body: decorateWorkspace(await storage.createExport(exportRecord, graphTrace('exported'), log), requestProvider, access.status),
    };
  }
