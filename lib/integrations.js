export async function createGitHubIssues(payload, { env = process.env, fetchImpl = fetch } = {}) {
  ...
  for (const issue of payload) {
    const { sourceTaskId, title, labels, body: issueBody } = issue;
    try {
      const response = await fetchImpl(`https://api.github.com/repos/${repository}/issues`, {
        ...
        body: JSON.stringify({ title, labels, body: issueBody }),
      });
      const responseBody = await safeJson(response);
      if (!response.ok) {
        results.push({ ok: false, title, sourceTaskId, status: response.status, error: responseBody.message || 'GitHub issue creation failed' });
      } else {
        results.push({
          ok: true,
          title,
      ...
      results.push({ ok: false, title, sourceTaskId, status: 'network', error: error.message });
    }
  }
  return results;
}
...
export async function createLinearIssues(payload, { env = process.env, fetchImpl = fetch } = {}) {
  ...
  for (const issue of payload) {
    const { sourceTaskId, ...providerIssue } = issue;
    try {
        ...
        body: JSON.stringify({
          ...
          variables: {
            input: {
              ...
              description: enrichLinearDescription(providerIssue),
            },
          },
        }),
      });
      ...
      results.push({ ok: false, title: providerIssue.title, sourceTaskId, status: 'network', error: error.message });
    }
  }
  return results;
}
