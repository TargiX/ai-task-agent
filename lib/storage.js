...
function withProvider(workspace) {
  return { ...workspace, provider: providerStatus() };
}

function jsonStorage() {
  function dbFile() {
    return process.env.TASK_AGENT_DB_FILE || DEFAULT_DB_FILE;
  }
  ...
  return {
    async getWorkspace() {
      return withJsonLock(async () => {
        ...
        await writeDbRaw(db);
        return withProvider(currentJsonWorkspace(db));
      });
    },
    async resetWorkspace() {
      return withJsonLock(async () => {
        ...
        await writeDbRaw(next);
        return withProvider(currentJsonWorkspace(next));
      });
    },
    async listRuns() {
      return withJsonLock(async () => {
    ...
    async selectRun(runId) {
      return withJsonLock(async () => {
        ...
        return withProvider(currentJsonWorkspace(selected));
      });
    },
...
function d1Storage() {
  ...
  async function ensureWorkspace() {
    ...
    await d1Query(
      ...
      [DEFAULT_WORKSPACE_ID, 'Default workspace', new Date().toISOString()],
    );
  }
  ...
  async function loadRun(runId) {
    ...
    const [run] = await d1Rows('select * from agent_runs where id = ? limit 1', [runId]);
  ...
  return {
    async getWorkspace() {
      return withProvider(await loadRun(await currentRunId()));
    },
    async resetWorkspace() {
      await ensureWorkspace();
      ...
      return withProvider({ ...initialWorkspace(), runHistory: await listRuns() });
    },
    async listRuns() {
      return listRuns();
    },
    async selectRun(runId) {
      ...
      return withProvider(await loadRun(runId));
    },
    async saveRun({ idea, prd, tasks, graph, logs }) {
      await ensureWorkspace();
      const runId = crypto.randomUUID();
      ...
      await d1Query(
        ...
        [runId, DEFAULT_WORKSPACE_ID, idea, 'planned', JSON.stringify(graph), now, now],
      );
      await d1Query(
...
function supabaseStorage() {
  ...
  async function ensureWorkspace() {
    const { error } = await supabase
      ...
      .upsert({ id: DEFAULT_WORKSPACE_ID, name: 'Default workspace' }, { onConflict: 'id' });
