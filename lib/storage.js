...
export function getStorage(workspaceId = DEFAULT_WORKSPACE_ID) {
  ...
  return getStorageWithOptions(activeWorkspaceId);
}

export function getStorageWithOptions(workspaceId = DEFAULT_WORKSPACE_ID, options = {}) {
  const activeWorkspaceId = normalizeWorkspaceId(workspaceId);
  if (
  ...
  if (process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY) {
    return supabaseStorage(activeWorkspaceId, options.supabaseClient);
  }
...
function supabaseStorage(workspaceId = DEFAULT_WORKSPACE_ID, injectedClient = null) {
  const supabase =
    injectedClient ||
