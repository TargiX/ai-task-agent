...
export function providerStatus() {
  ...
  return {
    ...
    access: process.env.WORKSPACE_ACCESS_TOKEN ? 'guarded' : 'demo-open',
  };
}
...
export function productionSetup(provider = providerStatus()) {
  ...
  const groups = [
    {
      ...
      missing: accessReady ? [] : ['WORKSPACE_ACCESS_TOKEN'],
      alternatives: ['shared-access-token'],
      commands: ['npm run vercel:env:sync -- --allow-partial --only=WORKSPACE_ACCESS_TOKEN --apply'],
    },
    {
      id: 'durable-storage',
  ...
  return {
    groups,
    acceptedSecretSets,
    productionReady: storageReady && llmReady && issueExportReady && accessReady,
    launchCommand: 'npm run production:launch -- --apply --scope=targixs-projects',
    launchChecklist: [
      'Fill .env.production.local with WORKSPACE_ACCESS_TOKEN, durable storage, one live LLM set, and one issue export set.',
...
function accessPreflight(configured) {
  ...
  return {
    status: 'fallback',
    detail: 'Workspace routes are open for demo mode; set WORKSPACE_ACCESS_TOKEN before production.',
  };
}
