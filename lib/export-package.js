import { exportPayload, exportableApprovedTasks, issueExportMode } from './domain.js';
...
export function createIssueExportPackage(target, workspace, provider) {
  ...
  const exportableTasks = exportableApprovedTasks(target, workspace.tasks, workspace.exports);
  const payload = exportPayload(target, workspace.prd, exportableTasks);
  const blockedReason = payload.length
    ? ''
    : approvedTasks.length
      ? `All approved tasks have already been exported to ${target}.`
      : 'Approve at least one task before preparing an issue package.';
  ...
  return {
    ...
    mode: issueExportMode(target, provider),
    generatedAt: new Date().toISOString(),
    summary: {
      prdTitle: workspace.prd?.title || 'Untitled PRD',
      approvedCount: approvedTasks.length,
      pendingExportCount: exportableTasks.length,
      exportedCount: approvedTasks.length - exportableTasks.length,
      totalTasks: workspace.tasks.length,
      blockedReason,
    },
    payload,
    markdown: payload.length ? issuePackageMarkdown(target, workspace, exportableTasks, payload) : '',
  };
}
