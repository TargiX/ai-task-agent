...
export async function verifyRuntimeSetup(storage) {
  ...
  checks.push(verifyWorkspaceAccessGuard(preflight.provider));
...
function verifyWorkspaceAccessGuard(provider) {
  const guarded = provider.access === 'guarded';
  return {
    id: 'workspace-access',
    label: 'Workspace access',
    status: guarded ? 'ready' : 'fallback',
    detail: guarded
      ? 'Workspace data routes require an access token.'
      : 'Workspace data routes are open for demo mode; set WORKSPACE_ACCESS_TOKEN for production.',
    evidence: { access: provider.access || 'demo-open' },
  };
}
